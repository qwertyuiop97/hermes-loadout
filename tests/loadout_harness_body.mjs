...
const selectSaved = async (tree, value) => { await act(async () => { labeled(tree, 'Saved loadout').props.onChange({ target: { value } }) }) }
let tree
...
await selectSaved(tree, '')
const dirtyDialog = tree.root.findByProps({ role: 'dialog' })
assert(dirtyDialog.props['aria-labelledby'], 'Dirty prompt has an accessible name')
assert.equal(typeof dirtyDialog.props.onKeyDown, 'function')
await click(tree, 'Cancel')
assert.equal(labeled(tree, 'Loadout name').props.value, 'Research revised')
await selectSaved(tree, '')
await act(async () => { tree.root.findByProps({ role: 'dialog' }).props.onKeyDown({ key: 'Escape', preventDefault() {} }) })
assert.equal(labeled(tree, 'Loadout name').props.value, 'Research revised')
await selectSaved(tree, '')
assert(tree.root.findByProps({ role: 'dialog' }))
await click(tree, 'Discard draft')
assert.equal(tree.root.findAll(row => row.props && row.props['aria-label'] === 'Loadout name').length, 0, 'Discarding a dirty draft allows the picker change')
await selectSaved(tree, records[1].id)
assert.equal(labeled(tree, 'Loadout name').props.value, 'Research')
await change(tree, 'Loadout name', 'Research save failure stays')
failSave = true
await selectSaved(tree, '')
assert(tree.root.findByProps({ role: 'dialog' }))
await click(tree, 'Save draft')
assert.match(JSON.stringify(tree.toJSON()), /Disk unavailable/)
assert.equal(labeled(tree, 'Loadout name').props.value, 'Research save failure stays')
failSave = false
await click(tree, 'Save draft')
assert.equal(records[1].name, 'Research save failure stays')
assert.equal(tree.root.findAll(row => row.props && row.props['aria-label'] === 'Loadout name').length, 0, 'Saving from the dirty prompt completes the pending picker navigation')
await selectSaved(tree, records[1].id)
await change(tree, 'Loadout name', 'Research revised')
...
await click(tree, 'Save loadout')
assert.equal(records[1].name, 'Research revised')
await change(tree, 'Loadout name', 'Dirty copy heading')
await click(tree, 'Delete loadout')
assert(tree.root.findByProps({ role: 'dialog' }), 'Delete is guarded by the dirty prompt')
await click(tree, 'Discard draft')
assert(tree.root.findByProps({ role: 'dialog' }), 'Discarding before delete opens the delete confirmation')
await click(tree, 'Cancel')
assert.equal(records.length, 3)
