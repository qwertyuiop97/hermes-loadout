...
await act(async () => { labeled(tree, 'Absolute project folder path').props.onChange({ target: { value: '/fixture/project one' } }); button(tree, 'Review project folder').props.onClick() })
...
await act(async () => { labeled(tree, 'Absolute project folder path').props.onChange({ target: { value: '/fixture/different' } }) })
...
await act(async () => { labeled(tree, 'Absolute project folder path').props.onChange({ target: { value: '/fixture/project one' } }) })
...
await act(async () => {
  labeled(tree, 'Custom client ID (lowercase letters, numbers, hyphens)').props.onChange({ target: { value: 'custom' } })
  labeled(tree, 'Custom client display name').props.onChange({ target: { value: 'Custom fixture' } })
  labeled(tree, 'Custom skills directory (absolute path)').props.onChange({ target: { value: '/fixture/custom' } })
})
failCustom = true
