...
const PLUGIN_SRC = process.env.PLUGIN_SRC || '/Users/demo/Projects/skills-toggle/desktop/plugin.js'
...
ok(channel.registry.length === 3, `registers 3 contributions (got ${channel.registry.length})`)
const paneC = channel.registry.find(c => c.area === 'panes')
const pageC = channel.registry.find(c => c.area === 'routes')
const palC = channel.registry.find(c => c.area === 'palette')
...
ok(!!palC && palC.data.label === 'Skills: toggle…', 'palette command "Skills: toggle…" registered')
